  ...
  return (
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        {adding && (
          <div className="border border-brass/30 bg-brass/5 p-5 mb-8">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {draft.photo_path && (
                <img src={imageUrl(draft.photo_path)!} alt="" className="w-14 h-14 rounded-full object-cover border border-ink/10" />
              )}
            </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {members.map((m) => (
            <div key={m.id} className="border border-ink/10 bg-white/40 p-4 flex items-center gap-3">
              {m.photo_path ? (
                <img src={imageUrl(m.photo_path)!} alt="" className="w-14 h-14 rounded-full object-cover flex-shrink-0" />
              ) : (
                <div className="w-14 h-14 rounded-full bg-stoneDark flex-shrink-0" />
              )}
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate">{m.full_name}</div>
                <div className="text-xs text-ink/50 truncate">{m.position}</div>
              </div>
              <div className="flex flex-col items-end gap-1">
                {!m.is_active && <span className="text-xs bg-ink/10 text-ink/50 px-2 py-0.5 rounded-full">Inactive</span>}
          ...
          {!loading && members.length === 0 && (
            <p className="text-ink/50 text-sm">No team members yet — add one to get started.</p>
          )}
        </div>
      </div>
