...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en" className={`${display.variable} ${body.variable}`} suppressHydrationWarning>
      <head>
        {/* Applies the saved (or system) theme before React hydrates, so there's
            no flash of the wrong theme on first paint. Runs synchronously,
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `
              (function () {
                try {
                  var saved = localStorage.getItem('niglo_theme');
