        ...
        {posts.map((post) => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="group flex gap-6 items-start">
            {post.cover_image_path && (
              <div className="relative w-40 h-28 flex-shrink-0 bg-stoneDark overflow-hidden">
            ...
            <div>
              ...
              {post.excerpt && <p className="text-sm text-ink/60 line-clamp-2">{post.excerpt}</p>}
            </div>
          </Link>
        ))}
        {posts.length === 0 && <p className="text-ink/50">No articles published yet.</p>}
      </div>
    </div>
  );
}
