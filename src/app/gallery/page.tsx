      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {albums.map((album) => (
          <div key={album.slug} className="block">
            <div className="relative aspect-square bg-stoneDark mb-3 overflow-hidden">
            ...
            {typeof album.images_count === 'number' && (
              <div className="text-sm text-ink/50">{album.images_count} photos</div>
            )}
          </div>
        ))}
        {albums.length === 0 && <p className="text-ink/50">No albums published yet.</p>}
      </div>
    </div>
  );
}
