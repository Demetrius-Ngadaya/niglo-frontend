      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="text-xs"
          />
          {uploadingField === field && <span className="text-xs text-ink/40">Uploading…</span>}
        </div>
        {draft[field] && (
          <img src={imageUrl(draft[field])!} alt="" className="w-16 h-16 object-cover border border-ink/10 mt-2" />
        )}
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
          ...
          <div className="border border-brass/30 bg-brass/5 p-5 mb-8">
            ...
            {editingId ? (
              <div className="mb-4 border-t border-brass/20 pt-4">
                <div className="text-sm text-ink/60 mb-1">Additional Gallery Photos</div>
                <p className="text-xs text-ink/40 mb-3">Select several at once — shown on the project's detail page.</p>
                ...
                {uploadingGallery && <span className="text-xs text-ink/50 ml-2">Uploading…</span>}
              </div>
            ) : (
              <p className="text-xs text-ink/40 mb-4">Save this project first, then you can add gallery photos.</p>
            )}
        ...
        <div className="space-y-3">
          {projects.map((p) => (
            <div key={p.id} className="flex items-center gap-4 border border-ink/10 bg-white/40 p-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-ink/40">{p.location || 'No location set'}</div>
              </div>
          ...
          {!loading && projects.length === 0 && (
            <p className="text-ink/50 text-sm">No projects yet — add one to get started.</p>
          )}
        </div>
      </div>
    </>
