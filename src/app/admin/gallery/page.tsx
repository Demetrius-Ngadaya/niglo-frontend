      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        {adding && (
          <div className="border border-brass/30 bg-brass/5 p-5 mb-8">
            ...
            <div className="mb-4">
              <div className="text-sm text-ink/60 dark:text-stone/60 mb-2">Cover Photo</div>
              <div className="flex items-center gap-3">
                <input type="file" accept="image/*" onChange={handleCoverUpload} className="text-sm" />
                {uploadingCover && <span className="text-xs text-ink/50 dark:text-stone/60">Uploading…</span>}
                {draft.cover_image_path && (
                  <img src={imageUrl(draft.cover_image_path)!} alt="" className="w-14 h-14 object-cover border border-ink/10 dark:border-stone/10" />
                )}
              </div>
            </div>
            ...
            {editingId ? (
              <div className="mb-4 border-t border-brass/20 pt-4">
                <div className="text-sm text-ink/60 dark:text-stone/60 mb-1">Album Photos</div>
                <p className="text-xs text-ink/40 dark:text-stone/50 mb-3">Select several at once.</p>
                ...
                {uploadingPhotos && <span className="text-xs text-ink/50 dark:text-stone/60 ml-2">Uploading…</span>}
              </div>
            ) : (
              <p className="text-xs text-ink/40 dark:text-stone/50 mb-4">Save this album first, then you can add photos.</p>
            )}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {albums.map((album) => (
            <div key={album.id} className="border border-ink/10 dark:border-stone/10 bg-white/40 dark:bg-white/5 p-4">
              ...
              <div className="text-xs text-ink/40 dark:text-stone/50 mb-3">{album.images?.length ?? 0} photos</div>
              <div className="flex items-center gap-3">
                {!album.is_active && <span className="text-xs bg-ink/10 text-ink/50 dark:text-stone/60 px-2 py-0.5 rounded-full">Inactive</span>}
          ...
          {!loading && albums.length === 0 && (
            <p className="text-ink/50 dark:text-stone/60 text-sm">No albums yet — add one to get started.</p>
          )}
        </div>
      </div>
