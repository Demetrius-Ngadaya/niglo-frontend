  ...
  return (
    <article className="max-w-3xl mx-auto px-6 py-20">
      ...
      {post.cover_image_path && (
        <div className="relative aspect-video bg-stoneDark dark:bg-white/5 mb-10">
          <Image src={imageUrl(post.cover_image_path)!} alt={post.title} fill className="object-cover" />
        </div>
      )}

      {post.content && (
        <div className="prose max-w-none text-ink/80 dark:text-stone/80" dangerouslySetInnerHTML={{ __html: post.content }} />
      )}
    </article>
  );
}
