        ...
        {categories.map((cat) => (
          <div key={cat.id}>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(cat.items || []).map((item) => (
                <div key={item.slug} className="border border-ink/10 bg-white/40 p-6 flex flex-col">
                  {item.image_path && (
                    <button
                      onClick={() => setLightbox(item)}
                      className="relative w-full h-48 mb-4 bg-white flex items-center justify-center overflow-hidden group border border-ink/5"
                    >
