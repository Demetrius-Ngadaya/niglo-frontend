      ...
      <div className="space-y-20">
        {categories.map((cat, i) => (
          <div key={cat.slug} id={cat.slug} className="scroll-mt-24">
            ...
            {cat.description && <p className="text-ink/60 max-w-2xl mb-8">{cat.description}</p>}
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(cat.services || []).map((svc) => {
                ...
                return (
                  <div key={svc.slug} className="border border-ink/10 bg-white/40 p-6">
                    {svc.image_path && (
                      <button
                        onClick={() => gallery.length > 0 && setLightbox({ service: svc, index: 0 })}
                        className="relative aspect-video mb-4 bg-stoneDark w-full block group overflow-hidden"
                      >
                    ...
                    {svc.short_description && (
                      <p className="text-sm text-ink/60">{svc.short_description}</p>
                    )}
                    {svc.starting_price && (
              ...
              {(!cat.services || cat.services.length === 0) && (
                <p className="text-sm text-ink/40">No services listed yet.</p>
              )}
            </div>
