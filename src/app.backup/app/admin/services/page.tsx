  ...
  return (
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="space-y-4">
          {categories.map((cat) => (
            <div key={cat.id} className="border border-ink/10 bg-white/40">
              <div className="flex flex-wrap items-center gap-4 px-5 py-4">
                ...
                >
                  ...
                  <span className="text-xs text-ink/40">{cat.services_count ?? 0} services</span>
                  {!cat.is_active && (
                    <span className="text-xs bg-ink/10 text-ink/50 px-2 py-0.5 rounded-full">Inactive</span>
                  )}
                </button>
              ...
              {expandedCategoryId === cat.id && (
                <div className="border-t border-ink/10 px-5 py-5 space-y-3">
                  {services
                    .filter((s) => s.service_category_id === cat.id)
                    .map((svc) => (
                      <div key={svc.id} className="flex items-center gap-4 border border-ink/10 bg-white/60 p-3">
                        {svc.image_path && (
                        ...
                        <div className="flex-1">
                          ...
                          {svc.short_description && <div className="text-xs text-ink/50">{svc.short_description}</div>}
                        </div>
                        {svc.featured && <span className="text-xs bg-brass/20 text-brass px-2 py-0.5 rounded-full">Featured</span>}
                        {!svc.is_active && <span className="text-xs bg-ink/10 text-ink/50 px-2 py-0.5 rounded-full">Inactive</span>}
                  ...
                  {services.filter((s) => s.service_category_id === cat.id).length === 0 && !addingServiceFor && (
                    <p className="text-sm text-ink/40">No services in this category yet.</p>
                  )}
                    ...
                    <div className="border border-brass/30 bg-brass/5 p-4 mt-2">
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {uploadingImage && <span className="text-xs text-ink/50">Uploading…</span>}
                          {serviceDraft.image_path && (
                            <img src={imageUrl(serviceDraft.image_path)!} alt="" className="w-10 h-10 object-cover border border-ink/10" />
                          )}
                        </div>
                      </div>
                      ...
                      {editingServiceId ? (
                        <div className="mb-4 border-t border-brass/20 pt-4">
                              ...
                              return (
                                <div key={img.id} className="relative group">
                                  <img
                                    ...
                                    alt=""
                                    className={`w-20 h-20 object-cover border-2 ${isCover ? 'border-brass' : 'border-ink/10'}`}
                                  />
                                  {isCover && (
                          ...
                          {uploadingGalleryImage && <span className="text-xs text-ink/50 ml-2">Uploading…</span>}
                        </div>
                      ) : (
          ...
          {!loading && categories.length === 0 && (
            <p className="text-ink/50 text-sm">No categories yet — add one to get started.</p>
          )}
        </div>
      </div>
