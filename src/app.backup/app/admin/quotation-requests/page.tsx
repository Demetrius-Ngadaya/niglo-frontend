...
export default function QuotationRequestsPage() {
  return (
    <Suspense fallback={<div className="max-w-6xl mx-auto px-6 py-16 text-ink/50 text-sm">Loading…</div>}>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          <button
            ...
            className={`px-4 py-2 text-sm border ${statusFilter === '' ? 'bg-ink text-stone border-ink' : 'border-ink/15 hover:border-brass'}`}
          >
            All
          </button>
          {STATUSES.map((s) => (
            <button
              ...
              className={`px-4 py-2 text-sm border capitalize ${statusFilter === s ? 'bg-ink text-stone border-ink' : 'border-ink/15 hover:border-brass'}`}
            >
              {s}
            </button>
          ))}
        ...
        {!loading && items.length === 0 && (
          <p className="text-ink/50 text-sm">No quotation requests found.</p>
        )}

        <div className="space-y-3">
          {items.map((item) => (
            <div key={item.id} className="border border-ink/10 bg-white/40">
              <button
                onClick={() => openRow(item)}
                className="w-full flex flex-wrap items-center gap-4 px-5 py-4 text-left hover:bg-white/60 transition-colors"
              >
                <span className="font-mono text-xs text-ink/50 w-32 flex-shrink-0">{item.reference_no}</span>
                <span className="font-medium flex-1 min-w-[140px]">{item.full_name}</span>
                <span className="text-sm text-ink/60 hidden sm:block">{item.phone}</span>
                <span className="text-sm text-ink/60 hidden md:block">{item.project_type}</span>
                {item.quote_total && (
              ...
              {expandedId === item.id && (
                <div className="border-t border-ink/10 px-5 py-6 space-y-6">
                  <div className="grid sm:grid-cols-2 gap-x-8 gap-y-2 text-sm">
                    <div><span className="text-ink/50">Email:</span> {item.email || '—'}</div>
                    <div><span className="text-ink/50">Location:</span> {item.location || '—'}</div>
                    <div><span className="text-ink/50">Budget:</span> {item.budget_range || '—'}</div>
                    <div><span className="text-ink/50">Preferred start:</span> {item.preferred_start_date ? new Date(item.preferred_start_date).toLocaleDateString() : '—'}</div>
                  </div>

                  <div>
                    <div className="text-ink/50 text-sm mb-1">Services requested:</div>
                  ...
                  {item.details && (
                    <div>
                      <div className="text-ink/50 text-sm mb-1">Details:</div>
                            ...
                            {r.path.toLowerCase().endsWith('.pdf') ? (
                              ...
                              <a href={imageUrl(r.path)!} target="_blank" rel="noopener noreferrer">
                                <img
                                  src={imageUrl(r.path)!}
                                  alt="Payment receipt"
                                  className="w-32 h-32 object-cover border border-ink/10 hover:opacity-90 transition-opacity"
                                />
                              </a>
                            )}
                  ...
                  <div className="border border-brass/30 bg-brass/5 p-5">
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="hidden sm:grid grid-cols-[1fr_80px_120px_120px_32px] gap-2 text-xs text-ink/50 px-1">
                      ...
                      {quoteItems.map((qi, i) => (
                        <div key={i} className="grid grid-cols-2 sm:grid-cols-[1fr_80px_120px_120px_32px] gap-2 items-center">
                          <input
                            ...
                            className="col-span-2 sm:col-span-1 border border-ink/15 px-3 py-2 bg-white text-sm focus-ring"
                          />
                          <input
                            type="number"
                            min={0}
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-4 border-t border-brass/20 pt-4">
                      <div>
                        <label className="text-ink/50 text-sm block mb-1">Valid until (optional)</label>
                        <input
                          type="date"
                      ...
                      <div className="text-right">
                        <div className="text-ink/50 text-sm">Total (TZS)</div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4 pt-2">
                    <div>
                      <label className="text-ink/50 text-sm block mb-1">Or just change status</label>
                      <select
                        value={draft.status}
                        onChange={(e) => setDraft((d) => ({ ...d, status: e.target.value }))}
                        className="w-full border border-ink/15 px-3 py-2 bg-white capitalize focus-ring"
                      >
                        {STATUSES.map((s) => (
                  ...
                  <div>
                    <label className="text-ink/50 text-sm block mb-1">Admin notes</label>
                    <textarea
                      value={draft.admin_notes}
                      onChange={(e) => setDraft((d) => ({ ...d, admin_notes: e.target.value }))}
                      rows={3}
                      className="w-full border border-ink/15 px-3 py-2 bg-white focus-ring"
