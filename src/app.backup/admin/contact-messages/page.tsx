      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        ...
        <div className="flex gap-2 mb-8">
          <button
            ...
            className={`px-4 py-2 text-sm border ${!unreadOnly ? 'bg-ink text-stone border-ink' : 'border-ink/15 hover:border-brass'}`}
          >
            All
          </button>
          <button
        ...
        <div className="space-y-2">
          {items.map((m) => (
            <div key={m.id} className={`border bg-white/40 ${m.is_read ? 'border-ink/10' : 'border-brass/40'}`}>
              <button onClick={() => openMessage(m)} className="w-full flex items-center gap-4 px-5 py-4 text-left hover:bg-white/60 transition-colors">
                ...
                <span className="text-sm text-ink/50 hidden sm:block">{m.subject || 'No subject'}</span>
              ...
              {expandedId === m.id && (
                <div className="border-t border-ink/10 px-5 py-5 space-y-3">
                  <div className="text-sm space-y-1">
                    <div><span className="text-ink/50">Email:</span> {m.email}</div>
                    {m.phone && <div><span className="text-ink/50">Phone:</span> {m.phone}</div>}
                    {m.subject && <div><span className="text-ink/50">Subject:</span> {m.subject}</div>}
                  </div>
          ...
          {!loading && items.length === 0 && (
            <p className="text-ink/50 text-sm">No messages found.</p>
          )}
        </div>
      </div>
    </>
