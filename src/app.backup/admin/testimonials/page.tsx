    ...
    <>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        {adding && (
          <div className="border border-brass/30 bg-brass/5 p-5 mb-8">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {draft.photo_path && (
                <img src={imageUrl(draft.photo_path)!} alt="" className="w-12 h-12 rounded-full object-cover border border-ink/10" />
              )}
            </div>
            ...
            <div className="flex items-center gap-4 mb-4 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="border border-ink/15 px-2 py-1 bg-white text-sm focus-ring"
                >
                  {[1, 2, 3, 4, 5].map((n) => (
        ...
        <div className="space-y-3">
          {items.map((t) => (
            <div key={t.id} className="flex items-start gap-4 border border-ink/10 bg-white/40 p-4">
              {t.photo_path ? (
                <img src={imageUrl(t.photo_path)!} alt="" className="w-12 h-12 rounded-full object-cover flex-shrink-0" />
              ) : (
                <div className="w-12 h-12 rounded-full bg-stoneDark flex-shrink-0" />
              )}
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium">
                  {t.client_name} <span className="text-brass">{'★'.repeat(t.rating)}</span>
                </div>
                <div className="text-xs text-ink/40 mb-1">{t.service_category || 'General'}</div>
                <p className="text-sm text-ink/70 line-clamp-2">{t.story}</p>
              </div>
              <div className="flex flex-col items-end gap-1 flex-shrink-0">
                <div className="flex gap-1">
                  {t.featured && <span className="text-xs bg-brass/20 text-brass px-2 py-0.5 rounded-full">Featured</span>}
                  {!t.is_published && <span className="text-xs bg-ink/10 text-ink/50 px-2 py-0.5 rounded-full">Hidden</span>}
                </div>
          ...
          {!loading && items.length === 0 && (
            <p className="text-ink/50 text-sm">No testimonials yet — add one to get started.</p>
          )}
        </div>
      </div>
    </>
