      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-ink/50 mt-2 max-w-lg">
        ...
        <div className="space-y-3">
          {slides.map((slide) => (
            <div key={slide.id} className="flex items-center gap-4 border border-ink/10 bg-white/40 p-3">
              ...
              <div className="flex-1">
                <div className="text-sm font-medium">{slide.title || <span className="text-ink/40">No title</span>}</div>
                <div className="text-xs text-ink/40">Order {slide.display_order}</div>
              </div>
              {!slide.is_active && <span className="text-xs bg-ink/10 text-ink/50 px-2 py-0.5 rounded-full">Inactive</span>}
          ...
          {!loading && slides.length === 0 && (
            <p className="text-ink/50 text-sm">No slides yet — add one to get started.</p>
          )}
        </div>
      </div>
    </>
