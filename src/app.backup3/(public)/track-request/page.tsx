...
export default function TrackRequestPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-20">
      ...
      <p className="text-ink/70 dark:text-stone/70 mb-12">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 mb-12">
        <input
          ...
          className="flex-1 border border-ink/15 dark:border-stone/15 px-4 py-3 bg-white/60 dark:bg-white/5 focus-ring"
        />
        <input
          name="phone"
          required
      ...
      {rentalResult && (
        <div className="border border-ink/10 dark:border-stone/10 bg-white/40 dark:bg-white/5 p-8 mb-8">
          <div className="flex items-baseline justify-between mb-8">
            <div>
              <div className="text-xs text-ink/50 dark:text-stone/60 mb-1">Reference</div>
              <div className="font-mono font-semibold">{rentalResult.reference_no}</div>
            </div>
            {rentalResult.rental_start_date && (
              <div className="text-right">
                <div className="text-xs text-ink/50 dark:text-stone/60 mb-1">Dates</div>
                    ...
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-4 h-4 rounded-full border-2 ${
                          i <= activeIndex ? 'bg-brass border-brass' : 'bg-white border-ink/20 dark:border-stone/20'
                        }`}
                      />
                      <div className="text-[11px] text-ink/60 dark:text-stone/60 mt-2 text-center w-16">
          ...
          <div className="text-sm text-ink/60 dark:text-stone/60 mb-2">Equipment requested:</div>
          <div className="space-y-1 mb-6">
            {rentalResult.items.map((item, i) => (
              <div key={i} className="text-sm bg-stoneDark dark:bg-white/5 px-3 py-2">
                {item.name} &times; {item.quantity}
              </div>
            ))}
          </div>

          <div className="text-xs text-ink/40 dark:text-stone/50">
      ...
      {result && (
        <div className="border border-ink/10 dark:border-stone/10 bg-white/40 dark:bg-white/5 p-8">
          <div className="flex items-baseline justify-between mb-8">
            <div>
              <div className="text-xs text-ink/50 dark:text-stone/60 mb-1">Reference</div>
            ...
            <div className="text-right">
              <div className="text-xs text-ink/50 dark:text-stone/60 mb-1">Project Type</div>
          ...
          {result.quote_items && result.quote_items.length > 0 && (
            <div className="mb-8 border-t border-ink/10 dark:border-stone/10 pt-6">
              <div className="eyebrow mb-4">Your Quotation</div>
              <table className="w-full text-sm mb-3">
                <thead>
                  <tr className="text-left text-ink/50 dark:text-stone/60 text-xs">
                ...
                <tbody>
                  {result.quote_items.map((item, i) => (
                    <tr key={i} className="border-t border-ink/5 dark:border-stone/10">
              ...
              <div className="mt-8 border-t border-ink/10 dark:border-stone/10 pt-6">
                <div className="eyebrow mb-3">Payment Receipts</div>

                {result.receipts && result.receipts.length > 0 ? (
                  <div className="space-y-3 mb-4">
                    {result.receipts.map((r) => (
                      <div key={r.id} className="flex items-center gap-3 border border-ink/10 dark:border-stone/10 bg-white/50 dark:bg-white/5 p-3">
                        {r.path.toLowerCase().endsWith('.pdf') ? (
                          ...
                          <a href={imageUrl(r.path)!} target="_blank" rel="noopener noreferrer">
                            <img
                              ...
                              className="w-16 h-16 object-cover border border-ink/10 dark:border-stone/10"
                            />
                          </a>
                        )}
                        <span className="text-xs text-ink/50 dark:text-stone/60">
                          Uploaded {new Date(r.uploaded_at).toLocaleDateString()}
                        </span>
                      </div>
                    ))}
                    <p className="text-xs text-ink/50 dark:text-stone/60">Made another payment? Upload another receipt below.</p>
                  </div>
                ) : (
                  <p className="text-sm text-ink/60 dark:text-stone/60 mb-3">
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-3">
                  <input
                    ...
                    className="flex-1 border border-ink/15 dark:border-stone/15 px-3 py-2 bg-white/60 dark:bg-white/5 text-sm focus-ring"
                  />
                  <button
          ...
          <div className="text-sm text-ink/60 dark:text-stone/60 mb-2">Services requested:</div>
