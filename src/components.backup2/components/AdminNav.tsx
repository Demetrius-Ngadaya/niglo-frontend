...
export default function AdminNav() {
  ...
  return (
    <div className="border-b border-ink/10 dark:border-stone/10 bg-white/40 dark:bg-white/5">
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between gap-4">
            ...
            <Link
              ...
              className={`py-4 text-sm font-medium border-b-2 -mb-px transition-colors ${
                pathname === link.href
                  ...
                  : 'border-transparent text-ink/60 dark:text-stone/60 hover:text-ink'
              }`}
            >
              {link.label}
            </Link>
        ...
        <button onClick={handleLogout} className="text-sm text-ink/60 dark:text-stone/60 hover:text-brass">
          Log Out
        </button>
      </div>
    </div>
